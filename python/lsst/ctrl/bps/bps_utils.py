...
def _make_id_link(config, run_id):
    """Make id softlink to the submit run directory.
    ...
    run_id : `str`
        WMS run ID.
    """
    if config.search("makeIdLink")[1]:
        if run_id is None:
            _LOG.warning("Run ID is None.  Skipping making id link.")
        else:
            _, submit_path = config.search("submitPath")
            # pathlib.Path.symlink_to() does not care if target exists
            if Path(submit_path).exists():
                _, id_link_path = config.search("idLinkPath", opt={"default": "${PWD}/bps_links"})
                _LOG.info("submit_path=%s, id_link_path=%s", submit_path, id_link_path)
                try:
                    id_link_path = Path(id_link_path)
                    id_link_path.mkdir(parents=True, exist_ok=True)
        ...
        _LOG.info("Not asked to make id link (%s)", config.search("makeIdLink")[1])
