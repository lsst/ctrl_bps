...
class PegasusWorkflow(object):
    def __init__(self, genWorkflow, config):
        ...
        self.pegasusFiles['dax'] = {'name': "%s/%s" % (self.workdir, 'workflow.dax'), 'obj': ADAG(self.config['workflowName'])}
    ...
    def _addActivatorToTC(self):
        ...
        for taskId in self.tasks:
            ...
            if computeSite not in alreadyAdded or taskAbbrev not in alreadyAdded[computeSite]:
                activator = Executable(taskAbbrev, arch=self.config['site'][computeSite]['arch'],
                                       os=self.config['site'][computeSite]['os'], installed=True)
    ...
    def _runPegasusPlan(self):
        ...
        cmd.append("--input-dir %s/input --output-dir %s/output"
