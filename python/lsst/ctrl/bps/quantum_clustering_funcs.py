...
def add_dim_clusters_dependency(
    ...
    for task_label in dim_labels:
        ...
        for node in qgraph.getNodesForTask(task_def):
            ...
            nodes_to_use = [node]
            nodes_already_visited: dict[UUID, bool] = {}
            while nodes_to_use:
                node_to_use = nodes_to_use.pop()
                match method:
                    case "source":
                        possible_nodes = qgraph.determineOutputsOfQuantumNode(node_to_use)
                    case "sink":
                        possible_nodes = qgraph.determineInputsToQuantumNode(node_to_use)
                for possible_node in possible_nodes:
                    if possible_node.taskDef.label in ordered_tasks[cluster_label]:
                        if possible_node.nodeId not in nodes_already_visited:
                            _LOG.debug(
                            ...
                            nodes_already_visited[possible_node.nodeId] = True
