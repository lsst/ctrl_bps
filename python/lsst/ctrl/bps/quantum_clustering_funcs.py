...
def _get_dim_config_settings(
    ...
    if "equalDimensions" in cluster_config:
        equal_dims = [pt.strip().split(":") for pt in cluster_config.get("equalDimensions").split(",")]
...
def partition_cluster_values(
    ...
    if partition_max_clusters:
        ...
        print(f"max_chunks = {partition_max_chunks}")
    ...
    print(f"max_size = {max_size}, remainder = {remainder}")

    # Make partitions to be used across all clusters for this label.
    partition_key_to_id = {}
    partition_id = 0

    cnt_extra = 0  # how many partitions that had an extra added in order to handle remainder
    curr_max_size = max_size
    cnt = curr_max_size + 1  # how many values in current partition, set so enters if clause
    ...
    print(f"partition cnt = {partition_id}, total # clusters = {partition_id * num_cluster_values}")
