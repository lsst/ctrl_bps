...
class ClusteredQuantumGraph:
    ...
    def validate(self):
        ...
        num_qgraph = len(self._quantum_graph)
        num_cqgraph = len(node_ids)
        if num_qgraph != num_cqgraph:
            raise RuntimeError(
                f"Number of Quanta in clustered qgraph ({num_cqgraph}) does not equal number in quantum graph"
                f" ({num_qgraph})"
            )
